const cartReducer = (state, action) => {
    if (action.type === "ADD_TO_CART") {
      ...
      if (match.length !== 0 && product.id===match[0].id){
        ...
        return state.cart.map((currEleme, index)=>{
          console.log('currEleme',currEleme)
          if(product.id === currEleme.id){
            let previous_cart = [...state.cart]
            
            previous_cart[index] = {id: currEleme.id, name: currEleme.name, image: currEleme.image, 
            quantity: currEleme.quantity+amount, amount:currEleme.amount, stock: currEleme.stock}

                        console.log('previous_cart update baad', previous_cart)
                        console.log('if me state me kya hai',state)
    ...
    console.log('image aa rhi hai kya', new_state)
